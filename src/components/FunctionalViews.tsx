...
const Modal: React.FC<{ open: boolean; title: string; onClose: () => void; children: React.ReactNode; noScroll?: boolean; size?: string; maxHeight?: string }> = ({ open, title, onClose, children, noScroll = false, size = 'max-w-2xl', maxHeight = 'max-h-[calc(100vh-2rem)]' }) => {
  if (!open) return null;
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#0e2c40]/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={title}>
    <div className={`w-full ${size} border border-slate-200 bg-white shadow-2xl ${maxHeight} ${noScroll ? 'overflow-hidden' : 'overflow-y-auto'}`}>
  ...
  return (
    ...
    >
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="text-xs font-bold text-slate-600">Responsable<div className="mt-1"><ChoiceMenu value={assigneeEmail} onChange={setAssigneeEmail} ariaLabel="Responsable" options={[{ value: '', label: 'Por asignar' }, ...project.assignedStudents.map((student) => ({ value: student.email, label: student.name }))]} /></div></label>
        ...
        <label className="text-xs font-bold text-slate-600">Prioridad<div className="mt-1"><ChoiceMenu value={priority} onChange={(value) => setPriority(value as TaskPriority)} ariaLabel="Prioridad" options={taskPriorities.map((item) => ({ value: item, label: priorityLabel[item] }))} /></div></label>
      </div>
...
const ProjectEditForm: React.FC<{ project: Project; onDone: (project: Project) => void; onCancel: () => void }> = ({ project, onDone, onCancel }) => {
  ...
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    onDone({ ...draft, code: draft.code.trim(), companyName: draft.companyName.trim(), title: draft.title.trim(), challengeDescription: draft.challengeDescription?.trim() || undefined, progressPct: Math.max(0, Math.min(100, Number(draft.progressPct) || 0)), minStudents: Math.max(1, Number(draft.minStudents) || 1), maxStudents: Math.max(Number(draft.minStudents) || 1, Number(draft.maxStudents) || 1), aiType: draft.aiType.map((item) => item.trim()).filter(Boolean) });
  };
  return <form onSubmit={submit} className="space-y-4"><div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-600">Código<input required value={draft.code} onChange={(event) => update('code', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Organización<input required value={draft.companyName} onChange={(event) => update('companyName', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600 md:col-span-2">Nombre del proyecto<input required value={draft.title} onChange={(event) => update('title', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600 md:col-span-2">Descripción del reto<textarea value={draft.challengeDescription || ''} onChange={(event) => update('challengeDescription', event.target.value)} rows={3} className={`${inputClass} mt-1`} /></label></div><div className="grid gap-3 md:grid-cols-4"><label className="text-xs font-bold text-slate-600">Avance (%)<input type="number" min="0" max="100" value={draft.progressPct} onChange={(event) => update('progressPct', Number(event.target.value))} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Estado operativo<input value={draft.progressStatus || ''} onChange={(event) => update('progressStatus', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Riesgo<select value={draft.riskLevel} onChange={(event) => update('riskLevel', event.target.value as Project['riskLevel'])} className={`${inputClass} mt-1`}><option value="verde">Verde</option><option value="amarillo">Amarillo</option><option value="rojo">Rojo</option></select></label><label className="text-xs font-bold text-slate-600">Complejidad<input type="number" min="1" max="10" value={draft.complexityRating} onChange={(event) => update('complexityRating', Number(event.target.value))} className={`${inputClass} mt-1`} /></label></div><div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-600">Mínimo de estudiantes<input type="number" min="1" value={draft.minStudents} onChange={(event) => update('minStudents', Number(event.target.value))} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Máximo de estudiantes<input type="number" min="1" value={draft.maxStudents} onChange={(event) => update('maxStudents', Number(event.target.value))} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600 md:col-span-2">Tipo de IA<input value={draft.aiType.join(', ')} onChange={(event) => update('aiType', event.target.value.split(','))} className={`${inputClass} mt-1`} placeholder="Generativa, clasificación…" /></label></div><details className="border border-slate-200 p-3"><summary className="cursor-pointer text-xs font-bold text-[#0e2c40]">Caracterización y enlaces</summary><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-slate-600">Impacto COP anual<input type="number" value={draft.copImpactAnnual || ''} onChange={(event) => update('copImpactAnnual', event.target.value ? Number(event.target.value) : undefined)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Impacto / descripción<input value={draft.copImpactDescription || ''} onChange={(event) => update('copImpactDescription', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Datos requeridos<input value={draft.requiredDatasets || ''} onChange={(event) => update('requiredDatasets', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Calidad y disponibilidad<input value={draft.dataQualityAvailability || ''} onChange={(event) => update('dataQualityAvailability', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Viabilidad técnica<input value={draft.techViability || ''} onChange={(event) => update('techViability', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Riesgos de IA<input value={draft.aiRisks || ''} onChange={(event) => update('aiRisks', event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">WhatsApp<input type="url" value={draft.whatsappUrl || ''} onChange={(event) => update('whatsappUrl', event.target.value || undefined)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Drive<input type="url" value={draft.driveFolderUrl || ''} onChange={(event) => update('driveFolderUrl', event.target.value || undefined)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">GitHub<input type="url" value={draft.githubUrl || ''} onChange={(event) => update('githubUrl', event.target.value || undefined)} className={`${inputClass} mt-1`} /></label><label className="text-xs font-bold text-slate-600">Teams<input type="url" value={draft.teamsMeetingUrl || ''} onChange={(event) => update('teamsMeetingUrl', event.target.value || undefined)} className={`${inputClass} mt-1`} /></label></div></details><div className="flex justify-end gap-2"><Button type="button" tone="secondary" onClick={onCancel}>Cancelar</Button><Button type="submit"><Save className="h-4 w-4" />Guardar cambios</Button></div></form>;
};
...
export const TasksView: React.FC<Common & { projectId?: string; isMonitor?: boolean }> = ({ projects, projectId, onChanged, onOpenProject, isMonitor = true }) => {
  ...
  const tasks = OperationsService.getTasks(projectId).filter((task) => statusFilter === 'todas' || (statusFilter === 'abiertas' ? task.status !== 'completada' : task.status === statusFilter)).sort((a, b) => Number(a.status === 'completada') - Number(b.status === 'completada') || priorityRank[b.priority] - priorityRank[a.priority] || (a.dueDate || '9999').localeCompare(b.dueDate || '9999'));
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); OperationsService.createIssue({ projectId: project.id, title: title.trim(), description: description.trim(), category, priority, reportedBy: 'Equipo del proyecto' }); onDone(); }} className="space-y-3 rounded-2xl border border-rose-100 bg-rose-50/50 p-4">
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="text-xs font-bold text-slate-600">Categoría<div className="mt-1"><ChoiceMenu value={category} onChange={(value) => setCategory(value as ProjectIssue['category'])} ariaLabel="Categoría" options={[{ value: 'tecnico', label: 'Técnico' }, { value: 'datos_accesos', label: 'Datos o accesos' }, { value: 'comunicacion', label: 'Comunicación' }, { value: 'equipo', label: 'Equipo' }, { value: 'recursos', label: 'Recursos' }, { value: 'otro', label: 'Otro' }]} /></div></label>
      </div>
      ...
      <label className="block max-w-xs text-xs font-bold text-slate-600">Prioridad<div className="mt-1"><ChoiceMenu value={priority} onChange={(value) => setPriority(value as TaskPriority)} ariaLabel="Prioridad" options={taskPriorities.map((item) => ({ value: item, label: priorityLabel[item] }))} /></div></label>
...
export const IssuesView: React.FC<Common & { projectId?: string; isStudent?: boolean }> = ({ projects, projectId, onChanged, onOpenProject, isStudent }) => {
  ...
  const [globalProject, setGlobalProject] = useState(projectId || projects[0]?.id || '');
  const project = projects.find((item) => item.id === (projectId || globalProject));
  const issues = OperationsService.getIssues(projectId).sort((a, b) => Number(a.status === 'resuelta') - Number(b.status === 'resuelta') || ({ critica: 4, alta: 3, media: 2, baja: 1 }[b.priority] || 0) - ({ critica: 4, alta: 3, media: 2, baja: 1 }[a.priority] || 0) || b.createdAt.localeCompare(a.createdAt));
        ...
        {issues.map((issue) => (
          <Card key={issue.id}>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <ChoiceMenu value={issue.status} onChange={(value) => { const status = value as ProjectIssue['status']; const resolution = status === 'resuelta' ? (issue.resolution || 'Incidencia marcada como resuelta.') : issue.resolution; OperationsService.updateIssue(issue.id, { status, resolution }); onChanged(); }} ariaLabel={`Estado de ${issue.title}`} className="w-40" options={[{ value: 'abierta', label: 'Abierta' }, { value: 'en_revision', label: 'En revisión' }, { value: 'esperando_tercero', label: 'Esperando tercero' }, { value: 'resuelta', label: 'Resuelta' }]} />
              {issue.status !== 'resuelta' && <Button tone="secondary" onClick={() => { OperationsService.updateIssue(issue.id, { status: 'resuelta', resolution: issue.resolution || 'Incidencia marcada como resuelta.' }); onChanged(); }}><Check className="h-4 w-4" />Marcar resuelta</Button>}
              {isStudent ? <span className="text-[11px] text-slate-500">Puedes actualizar el estado</span> : <Button tone="ghost" onClick={() => { if (window.confirm('¿Eliminar esta incidencia?')) { OperationsService.deleteIssue(issue.id); onChanged(); } }}><Trash2 className="h-4 w-4 text-rose-500" /></Button>}
...
const ProjectSummary: React.FC<{ project: Project; links: ProjectResourceLink[]; activity: ReturnType<typeof OperationsService.getActivity>; onAddLink: () => void; onContacts: () => void }> = ({ project, links, activity, onAddLink, onContacts }) => <div className="grid gap-4 lg:grid-cols-2"><Card className="lg:col-span-2"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-extrabold">Enlaces del proyecto</h2><div className="flex flex-wrap gap-2"><Button tone="secondary" onClick={onContacts}>Contactos de organización</Button><Button tone="secondary" onClick={onAddLink}>Nuevo enlace</Button></div></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{links.map((link) => <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 border border-slate-200 px-3 py-3 text-sm font-bold text-[#0e2c40] hover:bg-[#c1e1a7]"><LinkIcon className="h-4 w-4 text-[#148D8D]" />{link.label}</a>)}<button type="button" onClick={onContacts} className="flex items-center gap-2 border border-slate-200 px-3 py-3 text-left text-sm font-bold text-[#0e2c40] hover:bg-[#c1e1a7]"><Users className="h-4 w-4 text-[#148D8D]" />Contactos de organización <span className="ml-auto text-xs text-slate-500">{project.contacts.length}</span></button>{!links.length && <p className="text-sm text-slate-500">Aún no hay enlaces registrados.</p>}</div></Card><Card className="lg:col-span-2"><h2 className="font-extrabold">Actividad reciente</h2><div className="mt-3 divide-y divide-slate-100">{activity.map((item) => <div key={item.id} className="flex items-start justify-between gap-3 py-2 text-sm"><span>{item.message}</span><small className="whitespace-nowrap text-slate-400">{formatDate(item.createdAt)}</small></div>)}{!activity.length && <Empty text="La actividad aparecerá al crear tareas, incidencias, reuniones o documentos." />}</div></Card></div>;
...
export const PeopleView: React.FC<Common> = ({ projects, onChanged }) => {
  ...
  const clearProjectAssignment = () => {
    ...
    if (!assigned) { setManageOpen(false); return; }
    try {
      OperationsService.removeStudent(assigned.id, selectedStudent.email);
      setTargetProject('');
      setManageOpen(false);
      onChanged();
    } catch (caught) { window.alert(caught instanceof Error ? caught.message : 'No se pudo retirar al estudiante del proyecto.'); }
  };
  const deactivate = () => { if (!selectedStudent) return; const assigned = currentProject(selectedStudent); if (assigned) OperationsService.removeStudent(assigned.id, selectedStudent.email); SyncService.enqueueProfileActive(selectedStudent.id, false); setDeactivated((items) => [...items, selectedStudent.email.toLowerCase()]); setManageOpen(false); onChanged(); };
  return <div className="mx-auto max-w-6xl">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="flex w-full flex-1 flex-wrap items-center gap-3"><div className="relative min-w-[240px] flex-1 max-w-md"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre, código o correo" className={inputClass + ' pl-9'} /></div><ChoiceMenu value={projectFilter} onChange={(value) => setProjectFilter(value as 'todos' | 'con_proyecto' | 'sin_proyecto')} ariaLabel="Filtrar personas por proyecto" className="w-full min-w-[190px] sm:w-[210px]" options={[{ value: 'todos', label: 'Todos' }, { value: 'con_proyecto', label: 'Con proyecto' }, { value: 'sin_proyecto', label: 'Sin proyecto' }]} /></div><span className="text-xs text-slate-500">{visibleStudents.length} coincidencias</span></div>
    ...
    <Modal open={assignOpen} title="Asignar proyecto" size="max-w-2xl" onClose={() => setAssignOpen(false)}>{selectedStudent && <div className="min-h-[300px] space-y-4"><p className="text-sm text-slate-600"><b>{selectedStudent.name}</b><br />{selectedStudent.email}</p><label className="block text-xs font-bold text-slate-600">Proyecto<ProjectPicker projects={projects} value={targetProject} onChange={setTargetProject} /></label><div className="flex justify-end gap-2"><Button tone="secondary" onClick={() => setAssignOpen(false)}>Cancelar</Button><Button onClick={assign}>Guardar asignación</Button></div></div>}</Modal>
    <Modal open={manageOpen} title="Gestionar usuario" size="max-w-2xl" onClose={() => setManageOpen(false)}>{selectedStudent && <div className="min-h-[360px] space-y-4"><div className="border border-slate-200 bg-slate-50 p-4 text-sm"><b>{selectedStudent.name}</b><p className="mt-1 text-xs text-slate-600">Código: {selectedStudent.code || 'N/A'} · {selectedStudent.email}</p><p className="mt-1 text-xs text-slate-600">Proyecto actual: {currentProject(selectedStudent)?.code || 'Sin proyecto'}</p></div><label className="block text-xs font-bold text-slate-600">Cambiar proyecto<ProjectPicker projects={projects} value={targetProject} onChange={setTargetProject} /></label><div className="mt-3 flex flex-wrap justify-between gap-2"><div>{currentProject(selectedStudent) && <Button tone="secondary" onClick={clearProjectAssignment}>Sin proyecto</Button>}</div><div className="flex flex-wrap gap-2"><Button tone="secondary" onClick={() => { assign(); setManageOpen(false); }}>Guardar cambio</Button><Button tone="danger" onClick={deactivate}>Desactivar usuario</Button><Button tone="ghost" onClick={() => setManageOpen(false)}>Cancelar</Button></div></div></div>}</Modal>
  </div>;
};
