...
export const ProjectDetail: React.FC<Common & { projectId: string; onBack?: () => void; isMonitor?: boolean }> = ({ projects, projectId, onChanged, onOpenProject, onBack, isMonitor }) => {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <Modal open={editModal} title="Editar proyecto" onClose={() => setEditModal(false)}>{<ProjectEditForm project={project} onCancel={() => setEditModal(false)} onDone={(updated) => { OperationsService.updateProject(updated); setEditModal(false); onChanged(); }} />}</Modal>
