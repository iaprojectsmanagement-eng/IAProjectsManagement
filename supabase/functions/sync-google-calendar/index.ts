...
serve(async (request) => {
  ...
  try {
    ...
    const endpoint = action === "cancel" ? `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events/${eventId}` : `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events${eventId ? `/${eventId}` : ""}`;
    ...
    const body = method === "DELETE" ? undefined : JSON.stringify({ summary: meeting.title, description: `${meeting.agenda || ""}\n\nProyecto: ${meeting.projects?.title || ""}`.trim(), start: { dateTime: meeting.starts_at, timeZone: meeting.timezone || "America/Bogota" }, end: { dateTime: end, timeZone: meeting.timezone || "America/Bogota" }, attendees });
    ...
    const patch = action === "cancel" ? { google_event_id: null, calendar_sync_status: "sincronizado", calendar_event_url: null } : { google_event_id: event?.id ?? meeting.google_event_id, calendar_sync_status: "sincronizado", calendar_event_url: event?.htmlLink ?? meeting.calendar_event_url };
