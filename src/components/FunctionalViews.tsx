...
export const PeopleView: React.FC<Common> = ({ projects, onChanged }) => {
  ...
  const visibleStudents = students.filter((student) => student.isActive !== false && !deactivated.includes(student.email.toLowerCase()) && `${student.name} ${student.email} ${student.code || ''}`.toLowerCase().includes(query.toLowerCase()));
